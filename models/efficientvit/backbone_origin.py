...
class EfficientViTBackbone(nn.Module):
    def __init__(self, width_list: List[int], depth_list: List[int], in_channels=3, dim=32, branch=False, expand_ratio=4, split_height=8, norm="bn2d", act_func="hswish") -> None:
        super().__init__()
        self.branch = branch
        self.width_list = []
    ...
    def forward(self, x: torch.Tensor, fix_bn=True) -> Dict[str, torch.Tensor]:
        ...
        if not self.branch:
            ...
            for stage_id, stage in enumerate(self.stages, 1):
                if stage_id in (1,2):
                    concat_tensor = self.split_and_concat(x=concat_tensor, split_height=concat_tensor.shape[2] // self.split_height, ops=stage, n=self.split_height)
                    output_dict["stage%d" % stage_id] = concat_tensor
                elif stage_id == 3:
