#import torch
# origin_pth = torch.load('../checkpoints/effvit/eff_b0.pth' ,map_location=torch.device('cuda'))
#origin_pth = torch.load('./pose.pth' ,map_location=torch.device('cuda'))
from collections import namedtuple

from collections import namedtuple

# 定义 ADEClass
ADEClass = namedtuple('ADEClass', ['name', 'id', 'train_id', 'category', 'category_id', 'has_instances', 'ignore_in_eval', 'color'])

# 所有类别名称
classes_names = ("wall", "building, edifice", "sky", "floor, flooring", "tree",
...
# 合法类别标签
valid_classes = [1, 4, 6, 7, 9, 13, 15, 16, 18, 19, 20, 22, 23, 24, 25, 31, 32, 34, 40, 42, 44,
                 59, 65, 67, 68, 72, 74, 75, 76, 79, 83, 87, 90, 93, 99, 100, 101, 109, 112, 116,
                 121, 148]

# 为每个类别构造 ADEClass
classes = []
for idx in range(1, len(classes_names) + 1):
    name = classes_names[idx - 1]
    color = class_colors[idx - 1]
    if idx in valid_classes:
        ...
        ignore_in_eval = True
    has_instances = False
    classes.append(ADEClass(name, idx, train_id, category, category_id, has_instances, ignore_in_eval, color))
